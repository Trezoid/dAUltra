  ...
  function updateAlert(latestVer)
  {
    ...
    changeButton.addEventListener("click",
       ...
       function initSettings()
       {
         ...
         var tooManyAs = document.getElementsByTagName("a");
         for(var a = 0; a < tooManyAs.length; a++)
         {
            ...
            var tooManyDivs = document.getElementsByTagName("div");
         ...
         for(var d = 0; d < tooManyDivs.length; d++)
         {
          ...
          settingsLink.addEventListener("click", settingsModal, false);
          }
       function aboutBox()
       {
 ...
 ".a-stream * a.lit{margin:0!important;margin-left:0px!important;}"+
  ...
  function groupLog()
  {
    ...
    for(var i = 0; i < deviant.childNodes.length; i++)
    {
      ...
      {
        var groupID = deviant.childNodes[i].innerHTML.split("</i>");
  ...
  {
    if(document.URL.indexOf("http://my.deviantart.com/messages/#") != -1)
    {
      var headers = document.getElementsByTagName("h2")
      
      for(var i = 0; i < headers.length; i ++)
      {
        if(headers[i].className == "mczone-title")
        {
          var icon = headers[i];
          i = headers.length;
        }
  ...
  function suggestDD()
  {
    ...
    var box = document.getElementById("gmi-ResourceViewFaveButton");
    if(box != null)
    {
  ...
  function notDD()
  {
    var allis = document.getElementsByTagName("div");
    for(var i = 0; i < allis.length; i++)
    {
      if(allis[i].className == "dd-tile")
      {
    ...
    GM_xmlhttpRequest(
    {
      ...
      url: DDTab,
      onload: function(responseDetails) {
      var allDats = responseDetails.responseText.split("ppp")

      var lastDate = allDats[1].split("stream");

      var grabDate = lastDate[1];
    ...
    function lastDD(grabDate)
    {
      ...
      if(lastDD != null)
      {
      var gotDate = lastDD[1].split(" on ");
      var date = gotDate[1].split("</small>");
    ...
    if (totalLeft > 180)
    {
     ...
     timePassed = 180 - totalLeft;
              if((currentYear - year) <2)
      {
            ...
            suggButt.addEventListener('click',ohNoesAlert, false);
            }
    }
    }
